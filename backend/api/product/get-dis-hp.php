...
// Modify your SQL query to fetch products based on the specified criteria
$sql = "SELECT product.pro_id, product.pro_name, product.pro_price, product.pro_cal, product.pro_des, product.pro_dis, product.pro_img, category.cat_name AS cat_name 
    ...
    WHERE product.pro_dis > 0 AND category.cat_name = 'Discount'
    -- WHERE product.pro_dis > 0 AND (category.cat_name = 'Discount' OR category.cat_name = 'Popular')
...
if ($result->num_rows > 0) {
    ...
    $response['message'] = 'No products found with discount and in either the "Discount" or "Popular" category';
}
